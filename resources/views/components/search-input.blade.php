...
<div class="wbtr:text-gray-300 wbtr:flex wbtr:items-center wbtr:gap-4 wbtr:font-semibold">
  <div class="wbtr:relative wbtr:flex-1">
    <input
      type="text"
      name="{{ $search_name ?? 's' }}"
      placeholder="{{ $search_placeholder ?? __('Search', 'webentor') }}"
      class="wbtr:bg-gray-50 {{ $input_classes ?? '' }} wbtr:w-full wbtr:rounded-full wbtr:py-2.5 wbtr:pl-5 wbtr:pr-12 wbtr:text-current"
    ...
    >
      @svg('images.svg.search', 'wbtr:fill-current wbtr:w-5 wbtr:h-5')
    </button>
  </div>
